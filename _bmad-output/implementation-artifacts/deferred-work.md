...
- **D31 — `csp.export_to_sheets()` not implemented.** Last of the PRD's 10 public functions. YOLO-batch skipped because Google service-account JSON lives at `~/.config/csp/sa.json` (outside repo by design); without a real account I can't end-to-end test the Sheets push, and synthesizing `gspread` mocks would only verify the code doesn't crash, not that the wire format reaches Google correctly. NFR6 says Sheets push is async fire-and-forget — must not block daily-brief. **Lands with:** when Chris wants Sheets export — implement against `gspread` + `google-auth`, gate behind `Settings.google_sheet_id` + `Settings.google_sa_path`, raise `ConfigError` if either missing, write a respx-equivalent test against `googleapis.com` HTTP (or use `gspread`'s test helpers), and document the manual-cassette-recording step.
